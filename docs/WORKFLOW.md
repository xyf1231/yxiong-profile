...
> 网站是纯静态站点，内容和公开素材全部随项目一起由 Cloudflare Pages 托管。
...
- `papers/` 存放论文 PDF。
- `admin.html` 只在本地维护使用。
- `scripts/admin-server.mjs` 负责把后台修改写入本地项目。（当前部署通过 Vercel CLI 执行，计划迁移到 Cloudflare Pages。）
- 线上网站不连接数据库。
...
6. 点击"保存到本地"。
7. 本地预览确认无误。
8. 点击"发布"。（当前部署命令仍为 `vercel deploy`，托管目标已切换至 Cloudflare Pages。）
...
```bash
echo "v1.6.3" > VERSION && ./bump-version.sh
```
...
每次修改都要递增版本号，例如 `v1.6.2` -> `v1.6.3`。版本脚本会统一更新 HTML 引用、页脚和 `package.json`。
