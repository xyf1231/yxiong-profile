export const CUSTOM_STROKE_WIDTH = 2;

export type CustomLetterPath = {
  d: string;
  reverse?: boolean;
...
export function getCustomLetter(
  ...
  return { paths: entry.paths.map((p) => ({ ...p, strokeWidth })) };
}

/**
 * Optional hand-tuned SVG stroke paths per character, in the same coordinate
 * space as `layoutTextSegmented` (Hershey slot positions). Empty entries fall
 * back to smoothed Hershey geometry.
 */
export const customLetters: Record<string, CustomLetterEntry> = {
  a: {
    paths: [
  ...
  b: {
    paths: [
      { d: "M 14 4 C 12.6883 4.6558 11.5342 5.5114 10 5 C 8.483 4.4943 6.2253 0.1861 8.2477 0.1861 C 10.0633 0.1861 9.0922 2.5315 7.5327 4.6805 C 5.9689 6.8354 3.8124 8.7942 2.5407 8.992 C -0.0933 9.4016 0.9082 2.9916 1 1.798 C 1.2427 -1.3572 0.862 -12.202 6 -12.202 C 8.4714 -12.202 6.3998 -7.2016 6 -6.202 C 4.5767 -2.6439 2.1153 0.625 0 3.798" },
    ],
  },
  c: {
    paths: [
